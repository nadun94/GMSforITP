...
namespace GymMSystem.Buisness_Logic
{
    class finace_repository
    {
        ...
        public bool getIncomeOfOs(int year, int month, Finance ff,string service)
        {
            double amount;
            DataTable dt = new DataTable();
            ...
            try
            {
                ...
                string qurey = "SELECT SUM(amount) FROM  view_finance_aereobicFee where paid_date between @fdate and @tdate and service_type=@sty";
                ...
                cmd.Parameters.AddWithValue("@fdate", firstDayOfMonth.ToShortDateString());
                cmd.Parameters.AddWithValue("@tdate", lastDayOfMonth.ToShortDateString());
                cmd.Parameters.AddWithValue("@sty", service);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
